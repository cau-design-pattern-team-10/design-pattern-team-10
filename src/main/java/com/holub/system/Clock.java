...
public class Clock {
   ...
   * Start up the clock.
   *
   * @param millisecondsBetweenTicks The number of milliseconds between ticks. A value of 0
   *                                 indicates that the clock should be stopped.
   */
   ...
   * <PRE>
   * Clock.instance().addClockListener (	new Clock.Listener() {	public void tick()
  ...
  /**
   * Check if any item on the menu bar has been selected. This is an incredible kluge. The problem
   * is that Swing draws the menus on the same "canvas" as the main frame. As a consequence,
   * displayed menus are overwritten by a running game. Moreover, Swing provides no notification on
   * the order of "some menu item has been selected," so the only way to detect menu-bar activity is
   * to poll it. This method does just that, and returns true if some menu is being displayed. The
   * {@link #tick} method effectively suspends clock ticks as long as a menu is displayed, (which
   * slows down the clock, unfortunately).
   * <p>
   * From a design-patterns perspective, this method demonstrates that a facade (the MenuSite) does
   * not provide strict isolation from the underlying subsystem. I can't immagine another
   * application that would need to know whether or not the menu bar is active, so adding this menu
   * to the MenuSite would be "noise." Since the method does something that's of no relevance to
